"""
Video Object Detection Pipeline using YOLO-E Prompt-Free Model

This pipeline processes video files and performs object detection
without requiring text prompts. For each frame, it saves detection
...
class VideoDetectionPipeline:
    ...
    def process_video(self, video_path, save_output=True, frame_skip=1, save_detections=True):
        ...
        Args:
            ...
            save_detections: Whether to save detection labels and boxes as JSON
        ...
        if save_detections:
            detections_json = self.output_dir / f"{video_path.stem}_detections.json"
        ...
        stats = {
            ...
            "output_video": str(self.output_dir / f"{video_path.stem}_detected.mp4")
            if save_output
            else None,
...
def main():
    """Main entry point for the video detection pipeline."""
